  ...
  useEffect(() => {
    try {
      ...
      axios.get(baseURL + 'listings/my-messages/' + user_id + '/').then((res) => {
        ...
        console.log("number of sender's",res.data);
      })
    } catch (error) {
      console.log(error);
    }
            ...
            <div className="row g-0">
              ...
              <div className="px-4 ">
                  <div className="d-flfex align-itemfs-center">
                    ...
                    //    onClick={SearchUser} 
                       style={{border:"none", borderRadius:"50%"}}><i className='fas fa-search'></i></button>
                    </div> */}
                  </div>
                </div>
                {messages.map((message) =>
                  <Link 
                    to={"/messagedetails/" + (message.sender === user_id ? message.reciever : message.sender) + "/"}
                    // to={"/messagedetails/" + (message.sender)+ "/" }

                    href="#"
                    className="list-group-item list-group-item-action border-0"
                  >
                    <small><div className="badge bg-success float-right text-white">
                      {moment.utc(message.date).local().startOf('seconds').fromNow()}</div></small>
                    ...
                    {message.sender !== user_id && message.sender_profile?.photo && (
                    <img src={message.sender_profile.photo} className="rounded-circle mr-1" alt="1" width={40} height={40}/>
                    )}
                    {message.sender === user_id && message.reciever_profile?.photo && (
                    <img src={message.reciever_profile.photo} className="rounded-circle mr-1" alt="2" width={40} height={40}/>
                    )}
